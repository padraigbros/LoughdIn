...
async function accountDialog(){
  ...
  if(account){
    ...
    show();
    try{const conflicts=await sync.listConflicts();for(const conflict of conflicts){const row=element('div',{class:'sync-conflict'});row.append(element('p',{},'A change needs your choice. '+(conflict.kind||conflict.type||'Record')));const server=element('button',{class:'subtle-btn'},'Use synced version');server.onclick=async()=>{try{await sync.resolveConflict(conflict.opId,{strategy:'server'});row.remove();}catch(e){err.textContent=e.message;}};row.append(server);d.insertBefore(row,err);}}catch(e){err.textContent=e.message;}
    return;
  }
